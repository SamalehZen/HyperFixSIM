...
import type { CustomBoundarySet } from './nao-map-types'
...
export function buildInputObjectSchema(customSets?: CustomBoundarySet[]) {
  ...
  return z.object({
    query_id: z
      .string()
      .describe("The id of a previous `execute_sql` tool call's output to get data from."),
    ...
    latitude_key: z
      ...
      .optional()
      .describe(
        'Column with the latitude in WGS84 decimal degrees. Required for "points" and "scatter_bubble".'
      ),
    longitude_key: z
      .string()
      .optional()
    ...
    label_key: z
      ...
      .optional()
      .describe(
        'Column name used as the title of the popup shown when a point or region is clicked.'
      ),
    tooltip_keys: z
      .array(z.string())
      .optional()
...
export function buildInputSchema(customSets?: CustomBoundarySet[]) {
  ...
  const scatterBubbleSchema = base.required({
    latitude_key: true,
    longitude_key: true,
    size_key: true,
  })
