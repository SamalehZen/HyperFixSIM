...
import type * as displayChart from './display-chart-schema'
import type * as displayMap from './display-map-schema'
...
export function buildStoryChartBlock(input: StoryChartBlockInput): string {
  const xAxisKeyAttr = input.x_axis_key
    ? ` x_axis_key="${escapeDoubleQuotedStoryAttr(input.x_axis_key)}"`
    : ''
  const xAxisTypeAttr =
    input.x_axis_type != null
      ? ` x_axis_type="${escapeDoubleQuotedStoryAttr(input.x_axis_type)}"`
      : ''
  const xLabelAttr = input.x_axis_label
    ? ` x_axis_label="${escapeDoubleQuotedStoryAttr(input.x_axis_label)}"`
    : ''
  ...
  const yLabelAttr = input.y_axis_label
    ? ` y_axis_label="${escapeDoubleQuotedStoryAttr(input.y_axis_label)}"`
    : ''
  const yRightMinAttr =
    input.y_axis_right_min !== undefined ? ` y_axis_right_min="${input.y_axis_right_min}"` : ''
  const yRightMaxAttr =
    input.y_axis_right_max !== undefined ? ` y_axis_right_max="${input.y_axis_right_max}"` : ''
  ...
  const titleAttr =
    input.title != null && input.title !== ''
      ? ` title="${escapeDoubleQuotedStoryAttr(input.title)}"`
      : ''
...
export type StoryTableBlockInput = Pick<
  displayChart.TableInput,
  'query_id' | 'title' | 'conditional_formats'
>
...
export function buildStoryFilterBlock(input: StoryFilterBlockInput): string {
  ...
  const databaseIdAttr = input.database_id
    ? ` database_id="${escapeDoubleQuotedStoryAttr(input.database_id)}"`
    : ''
  const optionsAttr = input.options
    ? ` options='${escapeSingleQuotedStoryAttr(JSON.stringify(input.options))}'`
    : ''
