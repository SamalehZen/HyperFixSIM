...
const FILTER_BLOCK_REGEX =
  /\{%\s*filter\s+([A-Za-z_][A-Za-z0-9_]*)\s*%\}([\s\S]*?)\{%\s*endfilter\s*%\}/g
...
const INVALID_PLACEHOLDER_HINTS: Record<string, string> = {
  start:
    "date_range exposes a single {{ filters.<id>.sql }} value that already expands to 'start' AND 'end' — use AND col BETWEEN {{ filters.<id>.sql }}",
 ...
 */
export function validateSqlFilterTemplate(
  sql: string,
  options?: { knownFilterIds?: Iterable<string> }
): string[] {
  ...
  while ((placeholderMatch = outsidePlaceholderRegex.exec(sql)) !== null) {
    ...
    issues.push(
      ...describePlaceholderIssues(placeholderMatch[0], placeholderId, property, placeholderId)
    )
  }
