...
import type { SeriesConfig } from './display-chart-schema'
import type * as displayMap from './display-map-schema'
import type { MapType, RegionBoundaries } from './display-map-schema'
import { MapTypeEnum } from './display-map-schema'
...
export function resolveGridWidths(
  widthsAttr: string | undefined,
  childCount: number
): number[] | null {
  ...
  const isValid =
    widths.length === childCount && widths.every((width) => Number.isInteger(width) && width > 0)
...
export function resizeGridColumns(
  widths: number[],
  boundaryIndex: number,
  targetFraction: number
): number[] {
  ...
  const resizedWidths = cumulativeWidths.map(
    (boundary, index) => boundary - (cumulativeWidths[index - 1] ?? 0)
  )
...
export function setGridColumnsMarkup(rawGridContent: string, widths: number[]): string {
  ...
  const remainingAttributes = gridMatch[1]
    .replace(/\s+(?:widths|cols)\s*=\s*(?:"[^"]*"|'[^']*')/gi, '')
    .trim()
...
export function splitGridColumnsRaw(rawGridContent: string): {
  columns: string[]
  widths: number[]
} {
  const gridMatch = rawGridContent.match(/^<grid\b([^>]*)>([\s\S]*)<\/grid>\s*$/)
...
export function insertGridColumn(
...
export function popGridColumn(
  ...
  const remaining =
    remainingColumns.length >= 2
      ? buildGridMarkup(remainingColumns, remainingWidths)
      : remainingColumns[0]
...
export function popGridColumns(
  ...
  if (
    columns.length === 0 ||
    sorted.length === 0 ||
    sorted.some((index) => index < 0 || index >= columns.length)
  ) {
    return null
  }
  ...
  const popped =
    poppedColumns.length === 1 ? poppedColumns[0] : buildGridMarkup(poppedColumns, poppedWidths)
  return { remaining, popped }
}
